...
class nn():
    ...
    def load(cls, load_path):
        layers, w, b, epochs = cPickle.load(open(load_path, 'rb'))
        print ('Neural network loaded')
        return cls(layers = layers, w=w, b=b, previous_epochs=epochs)
    
    # Save nn
    def save(self, save_path):
        save_data = self.layers, self.w, self.b, self.epochs
    ...
    def fit(self, x, y, 
        ...
        et = datetime.now()
        print('------------')
        print('Training MSE\t', avg_error, '\t[ Done in', et-st, ']')
        print('------------')
    ...
    def predict(self, x):
        ...
        for test_idx in range(len(x)):
            ...
            pred = predict(x_value = x_value, w=self.w, b=self.b, layers = self.layers)
            preds.append(pred)
    ...
    def validate(self, x, y, verbose=True):
        ...
        for test_idx in range(len(x)):
            ...
            pred, error = predict(x_value = x_value, w=self.w, b=self.b, layers = self.layers, y_value = y_value)
            preds.append(pred)
            errors.append(error)
        # Print error
        et = datetime.now()
        print('\n-----------')
        print('Testing MSE\t', np.average(errors), '\t[ Done in', et-st, ']')
        print('-----------\n')        
